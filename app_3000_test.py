...
def draw_info_text(image, brect, handedness, hand_sign_text,
                   finger_gesture_text, mouse_sign_text):
    cv.rectangle(image, (brect[0], brect[1]), (brect[2], brect[1] - 42),
                 (0, 0, 0), -1)

    info_text = info_text1 = handedness.classification[0].label[0:]
    if hand_sign_text != "":
    ...
    if mouse_sign_text != "":
        info_text1 = info_text1 + ':' + mouse_sign_text
    cv.putText(image, info_text1, (brect[0] + 5, brect[1] - 24),
               cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv.LINE_AA)
...
def control_keyboard(most_common_keypoint_id, select_right_id, command, presstime, time_delta=1, keyboard_TF=True,
                     print_TF=False):
    if most_common_keypoint_id[0][0] == select_right_id and most_common_keypoint_id[0][1] == 5:
        if time.time() - presstime > time_delta:
            if keyboard_TF:
...
class WorkThread(QThread):
    ...
    def run(self):
        ...
        # ========= google 小姐 =========
        # speech_0 = gTTS(text="sleep mode", lang='ja')
        # speech_0.save('rest.mp3')
        # speech_0 = gTTS(text="鍵盤模式", lang='ja')
        # speech_0.save('keyboard.mp3')
        # speech = gTTS(text="滑鼠模式", lang='ja')
        ...
        while True:
            ...
            if left_id + right_id + mouse_id > -3:
                ...
                if most_common_ms_id[0][0] == 3 and most_common_ms_id[0][1] == 30:
                    ...
                    # 靜態手勢控制
                    presstime = control_keyboard(most_common_keypoint_id, 2, 'K', presstime)
                    ...
                    presstime = control_keyboard(most_common_keypoint_id, 9, 'C', presstime)
                    presstime = control_keyboard(most_common_keypoint_id, 5, 'up', presstime)
                    presstime = control_keyboard(most_common_keypoint_id, 6, 'down', presstime)
                    # presstime = time.time()
